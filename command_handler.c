...
const char VALID_CMD[] = {CMD_ZERO, CMD_CALIBRATE, CMD_RELMODE, CMD_ABSMODE, CMD_TOGMODE, CMD_GETPARAMS, CMD_GETRAW};
	...
	if (filled > (processed + CSI_LEN)) {
		...
		if (curChar == CMD_ZERO) {
			...
			if (flags & FL_PANOPERAND) {
				processed += 1;
				setPanShift(analogRead(PAN_PORT));
			} 
			if (flags & FL_TILTOPERAND) {
				processed += 1;
				setTiltShift(analogRead(PAN_PORT));
			}
